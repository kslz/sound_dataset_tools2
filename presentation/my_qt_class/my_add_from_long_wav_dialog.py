...
from application.services.input_service import InputByLongWavService
...
from presentation.pyuic.ui_AddFromLongWavDialog import Ui_AddFromLongWavDialog
from utils.chech_input_tools import check_is_type
from utils.tools import get_audio_duration


class AddFromLongWavDialog(BaseDialog):
    def __init__(self, parent, dataset_id):
        super().__init__(parent)
        # 使用ui文件导入定义界面类
        self.ui = Ui_AddFromLongWavDialog()
        # 初始化界面
        ...
        self.input_service = InputByLongWavService()
    ...
    def save_to_dataset(self):
        ...
        min_silence_len = self.ui.lineEdit_min_silence_len.text()
        non_silent_ranges = self.ui.lineEdit_non_silence_thresh.text()
        ...
        if speaker.strip() == "" or None:
            ...
            return
        if min_silence_len.strip() == "" or None:
            self.ui.error_lable.setText("最短静音长度为空")
            return
        if non_silent_ranges.strip() == "" or None:
            self.ui.error_lable.setText("静音阈值为空")
            return
        if not check_is_type(min_silence_len, int, "最短静音长度")[0]:
            self.ui.error_lable.setText("最短静音长度填写有误")
            return
        if not check_is_type(non_silent_ranges, int, "静音阈值")[0]:
            self.ui.error_lable.setText("静音阈值填写有误")
            return
        ...
        if os.path.isfile(wav_path):
            ...
            self.input_service.init_info(dataset_id=self.dataset_id,
                                         ...
                                         non_silent_ranges=int(non_silent_ranges),
                                         speaker=speaker,
            ...
            self.ui.error_lable.setText("音频文件不存在！")
            self.logger.error(f"音频文件 {wav_path} 不存在")
        pass
